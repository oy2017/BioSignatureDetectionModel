...
print("--- Loading and Preparing N2 Dataset ---")
df = pd.read_parquet('multirex_spectra_n2.parquet')
...
with open(os.path.join(results_path, 'n2_classification_report.txt'), 'w') as f:
    f.write(report_str)

cm = confusion_matrix(y_test, y_pred)
plt.figure(figsize=(8, 6))
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues')
plt.title('Confusion Matrix on N2 Dataset')
plt.savefig(os.path.join(results_path, 'n2_confusion_matrix.png'))
plt.close()

print("\n--- N2 Analysis Complete ---")
print(report_str)
