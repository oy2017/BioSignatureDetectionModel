...
# This script generates a multiverse of planetary systems with H2 as the fill gas.
...
print("Generating a balanced H2 dataset with 5000 universes...")
...
for item in generation_plan:
    ...
    print(f"--- Generating {count} H2 planets for profile: {profile_name} ---")
...
spectra_df.to_parquet("multirex_spectra_h2.parquet")

print("H2 dataset created and saved to multirex_spectra_h2.parquet")

# --- Validate Data ---
print("\n--- Validating H2 Dataset ---")
df = pd.read_parquet("multirex_spectra_h2.parquet")
if df.isnull().sum().sum() == 0:
    print("PASSED: No NaN values found in the H2 dataset.")
else:
    print("FAILED: NaN values found in the H2 dataset.")
