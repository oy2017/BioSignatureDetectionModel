...
for name, train_func in models_to_run.items():
    ...
    # Generate Plot
    print(f"--- Generating Scatter Corner Plot for {name} ---")
    
    figure = plt.figure(figsize=(15, 15))

    # This is the corrected plotting logic
    # Plot Incorrect points first, so they are more visible
    corner.corner(params_incorrect, fig=figure, labels=LABELS,
                  plot_contours=False, plot_density=False, plot_datapoints=True,
                  data_kwargs={'color': 'crimson', 'alpha': 0.7})
    # Overlay Correct points
    corner.corner(params_correct, fig=figure, labels=LABELS,
                  plot_contours=False, plot_density=False, plot_datapoints=True,
                  data_kwargs={'color': 'navy', 'alpha': 0.3})

    from matplotlib.lines import Line2D
    legend_elements = [Line2D([0], [0], marker='o', color='w', label='Correct', markerfacecolor='navy', markersize=10, alpha=0.5),
                       Line2D([0], [0], marker='o', color='w', label='Incorrect', markerfacecolor='crimson', markersize=10, alpha=0.8)]
    plt.legend(handles=legend_elements, loc='upper right', fontsize=14, bbox_to_anchor=(0.95, 0.95), frameon=False)
    plt.suptitle(f"Scatter Corner Plot of Prediction Errors ({name})", fontsize=20)
    
    filename = os.path.join(RESULTS_DIR, f'corner_plot_errors_scatter_{name.lower()}.png')
    plt.savefig(filename, dpi=300)
    plt.close()
    print(f"Plot saved to: {filename}")

print("\n--- All Scatter Corner Plots Generated ---")
