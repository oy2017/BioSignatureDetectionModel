...
parser.add_argument("--n_estimators", type=int, default=150, help="The number of trees in the forest.")
...
# --- 3. Train Classifier ---
print(f"--- Training RandomForest with n_estimators={n_estimators} and max_depth={max_depth} ---")
model = RandomForestClassifier(n_estimators=n_estimators, max_depth=max_depth, random_state=42, n_jobs=-1, class_weight='balanced')
...
model_suffix = f'_est_{n_estimators}_depth_{max_depth if max_depth is not None else "none"}'
report_filename = os.path.join(results_path, f'{fill_gas}_random_forest{pca_suffix}{model_suffix}_report.txt')
cm_filename = os.path.join(results_path, f'{fill_gas}_random_forest{pca_suffix}{model_suffix}_confusion_matrix.png')
cm_title = f'Confusion Matrix on {fill_gas} Dataset (Random Forest, PCA idx {pca_start_idx}-{pca_end_idx if pca_end_idx is not None else "end"}, Est={n_estimators}, Depth={max_depth if max_depth is not None else "None"})'
...
print(f"\n--- {fill_gas} Analysis Complete (Random Forest, PCA idx {pca_start_idx}-{pca_end_idx if pca_end_idx is not None else "end"}, Est={n_estimators}, Depth={max_depth if max_depth is not None else "None"}) ---")
print(report_str)