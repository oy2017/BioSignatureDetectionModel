...
# --- Argument Parser ---
parser = argparse.ArgumentParser(description="Evaluate a 1D ResNet on Raw Spectra.")
...
# --- 1. Load Data ---
print(f"--- Loading {fill_gas} Raw Data for ResNet ---")
...
def residual_block(x, filters, kernel_size=7):
    ...
    # Note: If input filters != output filters, we need to project the shortcut.
...
print(f"--- Building ResNet-like CNN (Input: {input_shape}) ---")
...
# --- 4. Train ---
print(f"--- Training ResNet for {args.epochs} epochs ---")
...
report_filename = os.path.join(results_path, f'{fill_gas}_resnet_cnn_raw_report.txt')
