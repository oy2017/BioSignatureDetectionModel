...
def build_cnn(input_dim, lr):
    tf.keras.backend.clear_session()
    # We use our standard CNN architecture from the master script
    model = Sequential([
...
def run_cnn_callback_search():
    print("==================================================================")
    print("  PHASE 2: LEARNING & CALLBACK OPTIMIZATION FOR CNN (R = 550)")
    print("==================================================================")
    ...
    for lr in learning_rates:
        for batch in batch_sizes:
            for patience in patience_values:
                # Instantiate CNN with current LR
                model = build_cnn(102, lr)
                es = EarlyStopping(monitor='val_loss', patience=patience, restore_best_weights=True)
                
                # Fit model on training set with dynamic EarlyStopping
                # CNN requires a reshaped 3D input: (samples, timesteps, features)
                model.fit(X_train.reshape(-1, 102, 1), y_train, epochs=150, batch_size=batch, validation_split=0.2, callbacks=[es], verbose=0)
                
                # Evaluate on the 5 independent test sets
                accuracies = []
                f1_scores = []
                for X_test, y_test in test_sets:
                    preds = (model.predict(X_test.reshape(-1, 102, 1), verbose=0) > 0.5).astype(int).flatten()
    ...
    print("==================================================================")
    print("  PHASE 2 CNN OPTIMIZATION COMPLETE")
    print("==================================================================")
    print(f"Optimal Learning Parameters found for CNN:")
