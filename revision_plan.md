...
been downgraded: R1-3 covers about 1.5 of seven requested axes, and R1-8's conclusion
...
**Status:** ◐ PARTIAL — `domain_shift_sweep.py`, `generate_cloudy_testset.py`, `evaluate_cloudy.py`. **About 1.5 of seven requested axes covered.**
...
| 3 | Different cloud and haze prescriptions | ◐ | grey deck done and verified (`generate_cloudy_testset.py`); hazes blocked, see `CLOUDS_HANDOFF.md` |
...
**Outstanding — hazes.** Only one prescription was tested: grey, wavelength-independent, infinite optical depth. `FlatMieContribution` and `LeeMieContribution` are both unusable as currently called and are being carried into a separate work session. State and next steps in `CLOUDS_HANDOFF.md`, including the specific lead (`lee_mie_mix_ratio` does not appear in the contribution's fitting parameters) and the prediction being tested: because the classifier ignores PC0 and PC1, a haze acting as a continuum tilt should degrade performance considerably less than a grey deck at equal feature suppression, then an **independent radiative transfer code** (petitRADTRANS is open-source Python and a genuine alternative — days of work, not months), then **transfer learning H2→N2** (he names it explicitly; requires regenerating N2 at 550 bins, since the existing files are 200-bin).
