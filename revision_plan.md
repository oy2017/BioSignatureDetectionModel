...
prescriptions differ is an interpretive claim beyond what he requested, and
defending it would require the band-resolved, occlusion and vertical-structure
analyses to appear in the paper. Those analyses exist in the repository
(`analyze_band_suppression.py`, `analyze_o3_band_occlusion.py`,
`generate_confined_haze_paired.py`) and are consistent with a wavelength-
dependence explanation, but they are working material, not manuscript content.
State the measured difference and stop.
