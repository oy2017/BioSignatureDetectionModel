...
Highest-value missing items, ranked by return against effort: **cloud and haze prescriptions** (the dominant confounder in real transmission spectroscopy — a high cloud deck mutes features and mimics low abundance). MultiREx hardcodes its physics to absorption and Rayleigh scattering only, so clouds require a patch to the MultiREx fork; that patch has been written and verified against TauREx's `SimpleCloudsContribution`, but **no cloudy dataset has been generated and nothing has been run through the pipeline**, so this axis remains uncovered, then an **independent radiative transfer code** (petitRADTRANS is open-source Python and a genuine alternative — days of work, not months), then **transfer learning H2→N2** (he names it explicitly; requires regenerating N2 at 550 bins, since the existing files are 200-bin).
