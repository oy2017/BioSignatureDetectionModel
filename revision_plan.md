...
**What this supports:** the pipeline transfers across radiative transfer implementations at a cost of about four accuracy points, and is roughly four times more sensitive to the choice of molecular opacity data than to the choice of code. Training remains entirely within TauREx; only evaluation leaves it, which is what the reviewer's stated remedy asks for — "experiments using spectra generated by independent radiative transfer codes, alternative molecular opacity databases".
...
finished spectra instead. The subsections below take them in that order.
...
**On axis 2, his own wording.** Listing what real Ariel data will contain, he names "incomplete molecular opacity databases" alongside the systematics and aerosols. The opacity swap therefore answers a concern he raised twice — once in the axis list and once in the description of real observations — and is the closest thing in this work to a direct reply to that sentence. Say so in the response document; it costs nothing and shows the experiment was chosen against his text rather than for convenience.
...
#### Injected systematics — what was run
...
No mechanism is proposed for the divergence. The reviewer asked for cloud and
haze prescriptions and for the resulting degradation; explaining *why* the two
...
> "The degradation does not arise from a loss of spectral signal: median feature amplitude is essentially unchanged. It is a shift in the decision boundary. Replacing the three non-ozone absorbers raises the predicted-positive rate from 49.5% to 61.8% against a true rate of 50.3%; replacing ozone as well lowers it to 44.0%. The direction therefore depends on which molecules are substituted, and performance under a change of opacity data cannot be assumed to fail in a predictable direction."

Then state the consequence plainly, because it bears on the paper's central claim:

> "Unlike the aerosol cases, where accuracy falls toward chance and the predicted probabilities reflect that loss of information, a change of opacity data leaves the model confident while shifting its decisions. Calibration established on one opacity compilation should not be assumed to hold under another."
...
> "A second aerosol prescription was tested. Photochemical haze was modelled with the Lee et al. (2013) parameterisation of Mie extinction for 0.1 µm particles, at five particle densities chosen so that the population feature suppression spans the grey deck's informative range. The haze degrades performance monotonically with density, and at matched feature suppression the two prescriptions differ: a thin haze costs less than a deck of equal muting, while at moderate optical depth the haze costs more — six accuracy points more at 0.58 of the clear feature amplitude."
