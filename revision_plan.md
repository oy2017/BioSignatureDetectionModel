...
The unwhitened MLP has a lower clean baseline (70.75% vs 77.35%), so the headroom normalisation in Result 2 is a modelling choice a reader could question. Result 1 — the raw crossover — does not depend on it, and both point the same way. Both networks are also **single training runs**; the aerosol restart study later measured ±1.7-point run-to-run scatter for this architecture. The 9.0-point final gap and the five-of-six-family pattern comfortably exceed that scatter, but the precise crossover point should be quoted as approximate ("near SNR 12"), not exact.
...
> "The reversal traces to the chromaticity of the haze. At matched overall suppression of 0.50x, the haze reduces the 0.5–1 µm band to 0.05x of its clear amplitude while leaving wavelengths beyond 1 µm at 0.79–1.06x, whereas the grey deck spreads its muting across the band. The haze therefore selectively suppresses the short-wavelength region — including the O₃ Chappuis band near 0.6 µm — while simultaneously imposing a steep blue continuum absent from the training distribution. That the hazy spectra retain more long-wavelength structure than deck spectra of equal overall amplitude, yet classify worse, indicates that the loss of total spectral information alone does not explain the degradation; the unfamiliar continuum shape and the selective loss of short-wavelength features, which these experiments do not disentangle, together account for the difference. The insensitivity to the leading components established in §4.2 protects against additive baseline shifts, but does not extend to multiplicative, wavelength-dependent opacity once it is optically significant. We note the two prescriptions also differ in vertical structure, so the attribution to wavelength dependence rests on the band-resolved amplitudes rather than on a single-variable comparison."
