...
class MLPClassifier(BaseEstimator, ClassifierMixin):
    """
    Custom Scikit-Learn wrapper for the specific 3-layer MLP architecture.
    Architecture: [256, 128, 64] with BatchNormalization and Dropout.
    """
    def __init__(self, input_dim=100, epochs=50, batch_size=128, 
    ...
    def fit(self, X, y):
        # Clear session to avoid clutter in loop
        ...
        # Fit the model
        # Note: We don't use early stopping here because GridSearchCV splits are cleaner without 
        # internal validation splits. We assume fixed epochs is the hyperparameter to tune.
...
def main():
    parser = argparse.ArgumentParser(description="GridSearchCV for 3-Layer MLP on PCA Spectra.")
    ...
    # Quick Mode: Subsample data
    if args.quick:
        print("--- QUICK MODE: Subsampling data and reducing grid ---")
        X_train = X_train[:500]
        y_train = y_train[:500]

    # --- 2. Define Pipeline ---
    # Pipeline: Scaler (Raw) -> PCA (100) -> Scaler (PCA) -> MLP
    pipeline = Pipeline([
    ...
    # Retrain on full training set is done automatically by GridSearchCV refit=True (default)
    ...
    report_filename = f'{fill_gas}_3layer_mlp_gridsearch_cv_report.txt'
    with open(os.path.join(results_path, report_filename), 'w') as f:
        f.write(f"GridSearchCV Results for 3-Layer MLP ({fill_gas})\n")
        f.write("==================================================\n")
