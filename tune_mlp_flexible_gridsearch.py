...
class FlexibleMLPClassifier(BaseEstimator, ClassifierMixin):
    """
    Custom Scikit-Learn wrapper for a Flexible MLP architecture.
    Allows tuning of hidden layers (depth/width) via GridSearchCV.
    """
    def __init__(self, input_dim=100, hidden_layers=(256, 128, 64), 
                 epochs=100, batch_size=64, learning_rate=0.001, 
                 dropout_rate=0.4, verbose=0):
    ...
    def fit(self, X, y):
        # Clear session to avoid memory leaks/clutter in loop
        tf.keras.backend.clear_session()
        
        self.model_ = Sequential()
        self.model_.add(Input(shape=(self.input_dim,)))
        
        # Dynamically add layers based on the tuple
        for units in self.hidden_layers:
            self.model_.add(Dense(units))
            self.model_.add(BatchNormalization())
            self.model_.add(Activation('relu'))
            self.model_.add(Dropout(self.dropout_rate))
            
        # Output Layer
        self.model_.add(Dense(1, activation='sigmoid'))
...
def main():
    parser = argparse.ArgumentParser(description="Flexible GridSearchCV for MLP Architecture & Params.")
    ...
    # --- 2. Define Pipeline ---
    # Scaler -> PCA -> Scaler -> MLP
    # We pass the pca_components to the wrapper so it knows input_dim
    pipeline = Pipeline([
        ('scaler_raw', StandardScaler()),
        ('pca', PCA(n_components=args.pca_components)),
        ('scaler_pca', StandardScaler()),
        ('mlp', FlexibleMLPClassifier(input_dim=args.pca_components, verbose=0))
    ])

    # --- 3. Define Grid ---
    if args.quick:
        param_grid = {
            'mlp__hidden_layers': [(128, 64)],
        ...
        param_grid = {
            'mlp__hidden_layers': [
                (256, 128, 64),          # Baseline (Best so far)
                (512, 256, 128),         # Deep & Wide
                (128, 128, 64, 32),      # Deep & Narrow
                (256, 128),              # Shallow
                (128, 64, 32)            # Compact
            ],
            ...
            'mlp__epochs': [100] # Fixed high epoch, assuming model stabilizes
        }
    ...
    report_filename = f'{fill_gas}_mlp_flexible_gridsearch_report.txt'
    with open(os.path.join(results_path, report_filename), 'w') as f:
        f.write(f"Flexible MLP GridSearchCV Results ({fill_gas})\n")
        f.write("==============================================\n")
        ...
        f.write("Full Grid Results (Top 15):\n")
        
        cv_results = pd.DataFrame(grid_search.cv_results_)
        top_results = cv_results.sort_values('mean_test_score', ascending=False).head(15)
