...
LINES = [("norm_xgb", "XGBoost, normalized"), ("norm_mlp", "MLP, normalized"),
         ("pca_xgb", "XGBoost, PCA")]
REL = [("norm_xgb", "XGBoost"), ("norm_rf", "Random Forest"), ("norm_mlp", "MLP")]


def main():
    fig, (a, b) = figure(0.41, ncols=2, gridspec_kw={"width_ratios": [1.05, 1]})
    summ = {}
    for cfg, _ in CFG:
    ...
    for k, (key, name) in enumerate(LINES):
        ...
        if key != "norm_mlp":          # within 0.2 pts of norm_xgb; labelling both only collides
            for x, v in zip(xs, vals):
                a.annotate(f"{v:.1f}", (x, v), textcoords="offset points", xytext=(0, 7),
                           ha="center", fontsize=7, color=INK2)
    a.set_xticks(xs); a.set_xticklabels([lab for cfg, lab in CFG if cfg in summ])
    a.set_ylabel("Five-set mean accuracy (%)"); a.set_ylim(70, 100); a.legend(loc="lower right", fontsize=7.5); panel_label(a, "A")
