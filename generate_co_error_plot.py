...
def get_data():
    ...
    co_vals = df_test['atm CO'].values
    
    return X_train_raw, y_train, X_test_raw, y_test, co_vals
...
def main():
    X_tr_raw, y_train, X_te_raw, y_test, co_vals = get_data()
    ...
    # --- Plotting ---
    print("--- Generating CO Line Plot ---")
    plt.figure(figsize=(10, 6))
    
    for m_name, y_pred in predictions.items():
        centers, errors = calculate_error_rates(y_test, y_pred, co_vals)
        sns.lineplot(x=centers, y=errors, label=m_name, marker='o', linewidth=2)
    
    plt.title('Error Rate vs Carbon Monoxide (CO) Concentration', fontsize=14, fontweight='bold')
    plt.xlabel('log(CO Abundance)', fontsize=12)
    plt.ylabel('Error Rate', fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    
    output_filename = os.path.join(RESULTS_DIR, 'error_vs_co_abundance.png')
