...
Each departure from the simulator re-rendered or modified the same test planets, so every loss was a within-planet difference against the same noise draw. Table 2 lists the departures. Hazes and clouds are common among observed hot Jupiters, whose spectra span a continuum from clear to strongly muted (9). Haze followed the Lee et al. prescription (30) with 0.1 µm particles. On the Rayleigh-enhancement scale used in retrievals of observed hot Jupiters (10), where the haze opacity at 0.35 µm is expressed as a multiple a of the H₂ Rayleigh opacity, the densities 3 × 10⁷, 2.4 × 10⁸ and 10¹⁰ m⁻³ correspond to log₁₀ a ≈ 2.4-2.7, 3.3-3.6 and 4.9-5.2 at 1 mbar. The ten hot Jupiters of that retrieval study spanned log₁₀ a = 2.1-5.7 (10), so 3 × 10⁷ m⁻³ lies at the weak end of the observed range. Unocculted spots used PHOENIX spectra (31) at a spot-to-photosphere temperature ratio of 0.85 (11).
