"""Figure 4: Tier 3 vs Tier 1 for the same mismatches — clean-trained screen, randomized screen, and
...
def main():
    ...
    for j, (cfg, tag, col) in enumerate((("ariel", "Tier 3 (102 points)", SERIES[0]), ("tier1", "Tier 1 (7 points)", SERIES[1]))):
        ...
        for i, (c, lab) in enumerate(CASES):
            ...
            a.bar(xi, fr, w * .92, color=col, alpha=.35, edgecolor=col, lw=.8, label=f"{tag}: clean-trained" if i == 0 else None)
