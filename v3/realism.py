"""v3 (C/O label) version. Three checks a referee asked for, each answering a specific attack.
...
def band_corr(cfg):
    """v3: correlation of each bin's normalised depth with log10(C/O), the label variable,
    and with the log abundances of the three carbon/oxygen carriers, so the index windows
    are chosen from the data rather than from a textbook."""
    X, y, P = load_split("train", cfg, noisy=False)
    Z = (X - X.mean(axis=1, keepdims=True)) / (X.std(axis=1, keepdims=True) + 1e-30)
    out = {}
    targets = {"C/O": np.log10(P["co_ratio"].to_numpy())}
    targets.update({g: P[f"atm {g}"].to_numpy() for g in ("CH4", "H2O", "CO")})
...
def floors(cfg, feats, model):
    ...
    # data-chosen C/O index: the six bins most positively correlated with log C/O minus the
    # six most negatively correlated, one number per spectrum, cut tuned on the training set
    wl, cc = band_corr(cfg)
    hi, lo = np.argsort(cc["C/O"])[-6:], np.argsort(cc["C/O"])[:6]
    def make(X):
        Z = (X - X.mean(axis=1, keepdims=True)) / (X.std(axis=1, keepdims=True) + 1e-30)
        return Z[:, hi].mean(1) - Z[:, lo].mean(1)
    Dtr = make(Xtr)
    best, cut = -1, None
    for a in np.quantile(Dtr, np.linspace(0.02, 0.98, 200)):
        acc = ((Dtr > a).astype(int) == ytr).mean()
        if acc > best:
            best, cut = acc, a
    res["C/O band index, data-chosen windows"] = [((make(X) > cut).astype(int) == y).mean() for X, y, _ in tests]
...
def realistic_subset(cfg, feats, model, depth_max=0.03, amp_max=3e-4):
    ...
    for t in TESTS:
        ...
        rows.append(dict(n=int(m.sum()), total=int(m.size), frac=float(m.mean()), accuracy=metrics(y[m], p)["accuracy"],
                         pos=float(y[m].mean())))
...
def main():
    ...
    L.append("")
    for g in ("C/O", "CH4", "H2O", "CO"):
        top = wl[np.argsort(cc[g])[-4:]]; bot = wl[np.argsort(cc[g])[:4]]
        L.append(f"   Bins most +correlated with log {g:<4}: {', '.join(f'{x:.2f}' for x in sorted(top))} um (max r = {cc[g].max():+.2f});"
                 f" most -correlated: {', '.join(f'{x:.2f}' for x in sorted(bot))} um (min r = {cc[g].min():+.2f})")
    L.append("")
    ...
    L += ["3. The frozen pipeline on the subset with mean transit depth below 3 % and",
          ...
          f"   planets retained: {sub['n'].sum()} of {sub['total'].sum()} ({sub['frac'].mean()*100:.1f} %), positive rate {sub['pos'].mean():.3f}",
