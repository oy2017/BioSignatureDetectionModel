...
- Figure 6 was regenerated after `fix_xgboost_corner.py` was aligned with the headline pipeline: all 102 components (it previously used PCs 2–101), 200 estimators and subsample 0.8 (previously 300 and 1.0), the five test sets pooled rather than one, and the same transit-depth cleaning as the rest of the pipeline. It now reports 88.58%, matching Section 4.2 and `H2_error_quintiles.txt`. The earlier version of this figure predated the current data files by six days and showed the physical parameters as uncorrelated clouds, which the delivered dataset does not reproduce — see the sampling row in Section 3.5.
