...
Everything under R1-1 (all seven robustness axes, two with stated approximations) and R1-3, R1-4, R1-5 (the PCA feature space) is complete. The remaining outstanding experiments are:

| # | Experiment | Comment | Why not done |
| :-- | :-- | :-- | :-- |
| 1 | Retrieval-derived labels | R1-2 | months of compute on the full benchmark; a 50–100-spectrum sample is the middle option |
| 2 | Whitening robustness on real observational spectra | R1-6 | no real Ariel data exists yet (the independent-code test *is* done) |
...
**How this is addressed.** Three threads, each detailed below: new domain-shift experiments (R1-1); the task definition conceded as a labelling convention rather than a detection — stated as a limitation and reworded, backed by two analyses (R1-2); and the PCA interpretation withdrawn and replaced with a measured result plus whitening ablations (R1-3 to R1-6).
...
**Claim (R1-1).** The classifier is an **in-domain triage tool** (88.9% accuracy), **not an Ariel-ready detector** — we concede that scope. But its out-of-domain failures are not opaque: across all seven experiments the degradation is graceful, and the three largest vulnerabilities each act through a distinct, *measured* mechanism — feature-amplitude suppression, chromatic stellar contamination, and opacity-driven threshold bias. Every experiment is evaluated on the frozen pipeline against the 88.9% baseline:

| # | Experiment | Result vs 88.9% | Reading |
| :-- | :-- | --: | :-- |
...
| 7 | Instrument noise (ExoRad2 / Ariel) | ~1 beyond white | minor; correlated noise dominates |

The three bolded rows are distinct failure mechanisms, each proven by more than the accuracy drop:

- **Feature-amplitude suppression** (clouds/hazes, high μ) — accuracy tracks amplitude down, 0.94× → 0.30× as the deck deepens.
- **Chromatic distortion** (stellar contamination) — feature amplitude stays flat (~0.04) while accuracy falls, so it adds structure rather than muting.
- **Decision-threshold bias** (opacity swap) — the predicted-positive rate moves 49.5% → 61.8% (true 50.3%) at near-constant amplitude: signal intact, boundary shifted.

Realistic Ariel noise coloring (Experiment 7) is a minor penalty — ~1 point beyond white at matched level; the dominant noise vulnerability is time-correlated systematics (Experiment 1, −22 at SNR 5), not photon-noise coloring.

**What we did — seven experiments.** In every case the trained pipeline is left untouched; only the evaluation data changes. Baseline throughout: 88.9% accuracy, Brier 0.080.
...
6. **Physical stellar contamination — real PHOENIX spectra.** Replaces Experiment 1's stellar-contamination proxy with the Transit Light Source Effect (Rackham et al. 2018): unocculted spots and faculae make the transit chord's stellar spectrum unrepresentative of the disk, multiplying the measured transit depth by a wavelength-dependent stellar contamination factor **ε(λ) = 1 / [1 − f(1 − F_spot(λ)/F_phot(λ))]** (f the covering fraction, F the stellar surface fluxes; faculae enter with an analogous −f_fac(1 − F_fac/F_phot) term). The surface fluxes are genuine BT-Settl PHOENIX model spectra (Allard et al. 2012; STScI grid), so the spot and faculae spectra carry their own molecular bands (TiO, water) that imprint structured, feature-like contamination a smooth proxy cannot. Cool spots (T_spot = 0.85 T_eff) and hot faculae (T_fac = T_eff + 100 K), solar metallicity, applied to the committed test spectra through the frozen pipeline. Accuracy 88.9% → 85.7% (2 % coverage) down to 65.7% (20 %); feature amplitude is essentially unchanged (~0.04 throughout), confirming this is **chromatic distortion, not muting** — a mechanism distinct from aerosols. **Caveat we state:** solar-metallicity BT-Settl and adopted spot/faculae contrasts; log g clamped to the grid ceiling (5.0) for the few compact stars. *Evidence:* [`evaluate_spots_phoenix.py`](../evaluate_spots_phoenix.py) → [`H2_spots_phoenix.txt`](../final_results/H2_spots_phoenix.txt).

7. **Realistic instrument noise — ExoRad2 with a published-parameter Ariel payload.** Replaces Experiment 1's parametric noise with the wavelength-dependent noise-to-signal ratio computed by ExoRad2 (Mugnai, Bocchieri & Pascale 2023), the open radiometric engine underneath ArielRad (Mugnai et al. 2020), driven by an Ariel payload reconstructed from published parameters (six channels: VISPhot, FGS1, FGS2, NIRSpec R 15, AIRS-CH0 R 100, AIRS-CH1 R 30; no atmospheric foreground, as befits an L2 telescope). The real noise is strongly colored — a 29.5× noise-to-signal span across 0.5–7.8 µm, concentrated in the AIRS bands that carry CH₄/CO₂/H₂O/O₃ — yet compared to white noise **at matched median level it costs only −0.8 to −1.3 points**. **Caveat we state:** the payload is literature-reconstructed, not the consortium's official ArielRad file (which is not public); a Planck SED per target Teff and a common operating point (1 R_sun, 20 pc) are used; the ExoRad-derived quantity applied is the noise's wavelength *shape*, with the overall median σ swept over 5×10⁻⁵, 1×10⁻⁴, 2×10⁻⁴ (matched between colored and white noise). *Evidence:* [`evaluate_ariel_noise.py`](../evaluate_ariel_noise.py) → [`H2_ariel_noise.txt`](../final_results/H2_ariel_noise.txt); payload in [`ariel_noise_model/`](../ariel_noise_model/).
...
**Physical stellar contamination** (PHOENIX Transit Light Source Effect, frozen pipeline; [`H2_spots_phoenix.txt`](../final_results/H2_spots_phoenix.txt)). "amp" is median feature amplitude — note it barely moves, so the loss is chromatic distortion, not muting:

| Stellar contamination (real PHOENIX) | amp | Accuracy | Change |
| :-- | --: | --: | --: |
...
**Realistic instrument noise** (ExoRad2 + published-parameter Ariel payload; [`H2_ariel_noise.txt`](../final_results/H2_ariel_noise.txt)). Colored ExoRad noise versus white noise **at the same median σ**, so the only difference is the wavelength coloring (29.5× across 0.5–7.8 µm):
...
The realistic photon/detector coloring is a minor additional penalty (~1 point); the overall noise *level* and time-correlated systematics (Experiment 1, −22 at SNR 5) remain the real noise vulnerabilities.

**Coverage of the seven axes.** "The seven axes" are the seven kinds of robustness test the reviewer explicitly lists in the last sentence of his comment above (independent code, alternative opacity, clouds/hazes, stellar contamination, resolution/SNR, instrument systematics, domain-shift/transfer-learning). All seven are now covered in full, two of them with stated approximations:
...
| 4 | Different stellar contamination models | ✅ full | Experiment 6 — physical PHOENIX spot/faculae model (TLSE); caveat: solar-Z, adopted contrasts | [H2_spots_phoenix.txt](../final_results/H2_spots_phoenix.txt) |
...
| 6 | More realistic instrumental systematics | ✅ full | Experiment 7 — ExoRad2 radiometric model; caveat: Ariel payload literature-reconstructed, not the official ArielRad file | [H2_ariel_noise.txt](../final_results/H2_ariel_noise.txt) |
| 7 | Domain-shift **or** transfer-learning | ✅ full | disjunctive requirement — the domain-shift half is covered by Experiments 1 & 2; transfer learning is the optional other disjunct | [H2_extrapolation_split.txt](../final_results/H2_extrapolation_split.txt) |

**Status.** *All 7 of the 7 axes done in full:* independent radiative transfer code (−4.1 points), alternative opacity data (−16.1, or −22.9 with ozone), cloud and haze prescriptions, resolution/SNR, domain shift (satisfies axis 7's disjunctive requirement), physical PHOENIX stellar contamination (−3.2 to −23.2), and ExoRad2 instrument noise (colored ~1 point beyond white). *Two carry stated approximations:* stellar contamination uses solar-metallicity PHOENIX with adopted spot/faculae contrasts, and the instrument-noise payload is reconstructed from published Ariel parameters rather than the consortium's official ArielRad file (which is not public).

**Questions for Mentor.** **Two axes rest on stated approximations — solar-Z PHOENIX spot contrasts, and a literature-reconstructed Ariel payload instead of the official ArielRad file. Is that fidelity sufficient to present these as complete, or would you want the official instrument file / PHOENIX metallicity grid pursued (e.g. via the Ariel consortium) before resubmission?**
...
**What we did.** *What we concede:* the class labels are a deterministic threshold rule (CH₄ ≥ 10⁻⁶ **and** O₃ ≥ 10⁻⁷) computed from the *generation parameters*, so the model recovers a labelling convention — not the observational detection Ariel would require, which needs retrieval with its attendant uncertainty, parameter degeneracy and abiotic alternatives. We do not contest this; we state it in the manuscript as an explicit limitation of the work. *What replaces the overclaim:* a reword that removes it, plus two analyses that quantify how far the convention governs the result — and retrieval-derived labels named as the required next step (below).

- **Reword (the largest part of the fix).** Rename the task wherever it describes what the model does — from "biosignature detection" to "recovery of an abundance-threshold labelling" — keeping "biosignature" only for the underlying scientific motivation. This costs nothing but words and stops the paper claiming more than the experiment shows.
...
> No principal component isolates the biosignature signal. Across all 102 components the largest single-component discriminative AUC is 0.66, and the two components carrying 98.4% of the spectral variance classify at chance and can be removed with no loss of accuracy — so the variance ordering is not an ordering of biosignature information. The molecular signals are not isolated in the low-variance tail either: no component is dominated by a single molecule (the component most associated with methane is only about 5% methane by variance), and although each molecule's spectral imprint is concentrated in only a few components, a large share falls on the overall-depth component (67% of the ozone imprint and 15% of the methane imprint sit on PC0) rather than being confined to the components previously attributed to chemistry. Classification succeeds only by nonlinearly combining many individually weak components — a linear classifier or a supervised projection on the same features reaches 68–74%, against 89% for the gradient-boosted trees. The whitening applied before the neural networks is therefore an optimization step for those scale-sensitive models, not a chemistry-selective operation: the tree ensemble is invariant to it and is the most accurate model, and no supervised re-weighting of the components improves on it.
...
**What we did — concede, and show the concession costs nothing.** *What we concede:* the manuscript's claim that whitening specifically amplifies the chemically informative components. It is not supported (whitening rescales every low-variance component equally, regardless of content), so we remove it and the "chemically relevant" language with it. *What replaces it:* whitening is described only as a plain optimisation step for the gradient-based models (the finding in the R1-3–R1-5 preamble). The experiment below is not a defence of the old claim — it establishes what whitening *actually* does and shows the concession does not weaken the paper. It crosses component range × whitening on/off for each model:
